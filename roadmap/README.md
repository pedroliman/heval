...
- Item 6, resonant engine names and clearer parameters ([06-resonant-api.md](06-resonant-api.md)): rename the engines to `MarkovModel`, `MicrosimModel`, and `DESModel`; rename the Markov `build` callback to `model_fn`; and collapse `discount_cost` and `discount_effect` into one `discount_rate` (default `0.03`, annual clock).
