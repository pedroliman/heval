...
The one-way and one-at-a-time outputs feed the existing `heormodel.report.tornado_data` and `plot_tornado`, generalized to accept a DSA result rather than only a PSA. The grid output supports a two-way heatmap of an outcome (for example, the ICER) across two parameters.

## Deliverables

- `heormodel.dsa` with `one_way`, `one_at_a_time`, `grid`, and the `Design`/`descriptor` contract, added to `__all__` and the quartodoc reference.
