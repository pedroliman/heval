...
- [Markov cohort model](tutorials/mdm-cohort.qmd): the standard state-transition model, then the [microsimulation engine](tutorials/microsim.qmd) as the more detailed alternative.
...
- [Surrogate-accelerated calibration](tutorials/surrogate-calibration.qmd): train a Gaussian process surrogate of a Markov model on a small design, calibrate it with the `sbi` package, and match the calibration workflow's posterior with about a hundred times fewer model runs.
