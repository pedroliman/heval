...
The constructor takes the model structure; `run_psa` takes the draw matrix. The two strategies are bare names, and the model functions branch on the strategy name (`strategy == "Treatment"`) to decide whether treatment is on. The two strategies share one population through common random numbers (the engine default), so the incremental result reflects the treatment effect rather than sampling noise.
...
## Branching on the strategy name versus using overrides

A strategy carries a name and, optionally, parameter overrides. Which to use depends on how the arms differ:

- Branch on the name when the arms differ in structure or in which model function runs, as here: treatment changes a utility and a cost, so the model reads `strategy == "Treatment"`. The comparison is not a number that lives in the `ParameterSet`.
- Use a `Strategy` with overrides for a numeric scenario knob the model already reads as a parameter, such as a server count `Strategy("Expanded", {"n_servers": 2})`. The discrete-event tutorial does this for clinic capacity.

Encoding an arm as a pseudo-parameter, a flag like `{"on_treatment": 1.0}` read back as a boolean, is the case branch-on-name replaces. That flag is not in the `ParameterSet`, so it is invisible to `evppi_ranking` and to the deterministic sensitivity builders, and a float stands in for a flag. The ranking below therefore reports only genuine parameters.
