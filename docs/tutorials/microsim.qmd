...
The constructor takes the model structure; `run_psa` takes the draw matrix. The two interventions are bare names, and the model functions branch on the intervention name (`intervention == "Treatment"`) to decide whether treatment is on. The two interventions share one population through common random numbers (the engine default), so the incremental result reflects the treatment effect rather than sampling noise.
