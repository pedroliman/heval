...
description: "Build one model as both a cohort trace and an individual simulation from the same rates, to cross-validate the engines and show what heterogeneity adds."
---
...
Competing hazards convert to a transition-probability row the same way in both engines, which is what `rows_from` computes. From a state with destination hazards $h_1, \dots, h_k$ and total $H = \sum_j h_j$, the probability of leaving within a cycle is $1 - e^{-H}$, split across destinations in proportion to their hazards,

$$P_j = \left(1 - e^{-H}\right)\frac{h_j}{H}.$$
