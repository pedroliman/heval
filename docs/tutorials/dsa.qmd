...
description: "Run the three deterministic sensitivity designs, one-way, one-at-a-time, and grid, and read them as tornado and heatmap reports."
keywords: [deterministic sensitivity analysis, DSA, one-way sensitivity analysis, one-at-a-time, tornado diagram, tornado plot, scenario analysis, cost-effectiveness, Python]
---

<!-- colab-badge:start -->

<a href="https://colab.research.google.com/github/pedroliman/heormodel/blob/main/docs/_notebooks/dsa.ipynb"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"></a>
...
`one_way` varies a single parameter across the listed values, holding the rest at base. The result each design reads is the incremental net monetary benefit of one intervention over its comparator, $\text{INMB} = \lambda\,\Delta E - \Delta C$, which is positive exactly when the intervention is cost-effective at the threshold $\lambda$. Here the incremental net monetary benefit of Intervention AB over standard of care falls as the treatment-B cost rises, crossing zero near its base value.
...
The utility of the treated Sick state (`u_trtA`) and the progression hazard ratio under treatment B (`hr_S1S2_trtB`) move the result most; the state costs move it least.
