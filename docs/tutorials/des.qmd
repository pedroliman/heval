...
The constructor takes the model; `run_psa` takes the draw matrix. The two strategies differ only in capacity, so each is a `Strategy` carrying the numeric knobs that arm sets: `n_servers`, and a per-patient overhead `c_capacity` for the expanded arm. This is the right use of overrides, a scenario knob the model already reads as a parameter, rather than a flag standing in for which arm is which. They see the same patients and service draws through common random numbers, so the incremental result reflects the capacity change, not sampling noise.

```{python}
from heormodel.models import DESModel, Strategy
...
engine = DESModel(
    ...
    strategies=[Strategy("Standard capacity", {"n_servers": 1, "c_capacity": 0.0}),
